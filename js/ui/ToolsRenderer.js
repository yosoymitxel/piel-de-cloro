...
export class ToolsRenderer {
    ...
    animateToolThermometer(value, container = null, isInfected = null) {
        const targetContainer = container || this.elements.npcDisplay;
        if (!targetContainer || (targetContainer.jquery && targetContainer.length === 0)) return;
        ...
        const fillColor = value < 35
            ? (isInfected ? State.colors.CHLORINE_SUTIL : State.colors.SAFE)
            : (isInfected ? State.colors.CHLORINE_LIGHT : '#a83232');
        ...
        const fill = $('<div>', {
            css: {
                ...
                filter: isInfected ? `drop-shadow(0 0 4px ${State.colors.CHLORINE_SUTIL})` : 'none'
            }
        });
    ...
    animateToolPupils(type = 'normal', container = null, isInfected = false) {
        ...
        setTimeout(() => {
            ...
            if (isInfected) {
                setTimeout(() => {
                    pupil.css('background', State.colors.CHLORINE_LIGHT);
